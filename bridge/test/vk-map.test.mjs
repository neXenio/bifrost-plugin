...
test('looks up by email claim first, then sub', (t) => {
  const file = tempMapFile({ 'alice@luca-app.de': 'test-key-alice', 'sub-bob': 'test-key-bob' });
  const map = createVkMap(file);
  t.after(() => map.close());

  assert.equal(map.lookup({ email: 'alice@luca-app.de', sub: 'sub-alice' }), 'test-key-alice');
...
test('reload picks up new entries', (t) => {
  ...
  map.reload();
  assert.equal(map.lookup({ email: 'carol@luca-app.de' }), 'test-key-carol');
...
test('email lookup is case-insensitive (sync job lowercases keys)', (t) => {
  ...
  assert.equal(map.lookup({ email: 'alice@luca-app.de' }), 'test-key-alice');
  assert.equal(map.lookup({ email: 'ALICE@LUCA-APP.DE' }), 'test-key-alice');
});
...
test('atomic rename replacement is picked up by reload (sync job write pattern)', (t) => {
  ...
  map.reload();
  assert.equal(map.lookup({ email: 'erin@luca-app.de' }), 'test-key-erin');
