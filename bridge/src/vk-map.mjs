 ...
 * then `sub`. An authenticated user with no mapping gets `null` — callers
 * must treat that as a hard deny, never fall back to a shared key.
 *
...
export function createVkMap(mapPath, { logger } = {}) {
  ...
  return {
    ...
    lookup(claims) {
      ...
      return users[email] ?? users[sub] ?? null;
    },
    size() {
      return Object.keys(users).length;
    },
